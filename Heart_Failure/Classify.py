...
df = pd.read_csv("heart_failure_pp.csv")
...
for i in range(len(cl_list)):
    ...
    cl = cl_list[i]
    scores = cross_val_score(cl, X_1, y, cv=10, scoring='accuracy')
    print("{} Ordinal Encoded".format(cl_strings[i]))
    print("Accuracy: %0.3f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
    cl_trained_list.append((scores.mean(), cl, 'oe'))

    scores = cross_val_score(cl, X_2, y, cv=10, scoring='accuracy')
...
# '''Fit the Classifier'''
# if best_cl[2] == 'oe':
#     train_cl = best_cl[1].fit(X_1, y)
# elif best_cl[2] == 'ohe':
#     train_cl = best_cl[1].fit(X_2, y)
# else:
